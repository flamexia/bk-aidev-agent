...
class CallBackView(View):
    ...
    async def reply_event(self, payload: dict):
    ...
    def reply_text(self, payload: dict):
        ...
        if content.lower() in ALL_PLUGIN:
            ...
            # 启动后台线程处理实际的AI请求
            thread = threading.Thread(target=self._process_ai_request_async, args=(content, stream_id), daemon=True)
            thread.start()

            # 立即返回"正在思考中...."的消息
            return stream_msg("正在思考中....", False, stream_id)

    def _process_ai_request_async(self, content: str, stream_id: str):
        """异步处理AI请求的后台方法"""
        try:
            start_time = time.time()
            first_response_time = None
            chat_root = (
                settings.BK_APIGW_MANAGER_URL_TMPL.format(api_name=settings.BKPAAS_BK_PLUGIN_APIGW_NAME)
                + "/"
                + "prod"
